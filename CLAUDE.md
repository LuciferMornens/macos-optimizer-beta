...
## 0) Explorer Usage
- **Use Explorer WITH Sonnet 4.5 model** -> Get all context needed, find all files needed, find anything in codebase. DO NOT SEARCH URSELF! Use Explorer. We are doing context priming here, where you are focusing only on receiving the right files / context and work on it. You can read the given files after explorer is done to understand(or the certain lines of code).
